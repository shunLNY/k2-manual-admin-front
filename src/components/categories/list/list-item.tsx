...
const ListItem = (props: Props) => {
	...
	const hasChildren =
		item.child_categories && item.child_categories.length > 0;
		...
		>
          ...
          />
				...
				<div className={styles.item_status}>
					{item.status === "private" && (
						<span
							className={`${styles[`border_${categoryStatus[0].color}`]}`}
						>
							{categoryStatus[0]?.text}
						</span>
					)}
					{item.status === "public" && (
						<span
							className={`${styles[`border_${categoryStatus[1].color}`]}`}
						>
							{categoryStatus[1]?.text}
						</span>
					)}
				</div>
				<div>
				...
				{level < 3 && (
					<div onClick={(e) => e.stopPropagation()}>
						<button
							...
							onClick={() =>
								router.push(
									`/categories/new?parentId=${item.id}`,
								)
							}
						>
							+サブカテゴリ
			...
			{(canExpand || level > 1) && hasChildren && isOpen && (
				<SortableContext
					items={item.child_categories.map((i: any) => i.id)}
					strategy={verticalListSortingStrategy}
				>
					<ul className={categoryListStyles.sortable_list}>
						{item.child_categories.map(
