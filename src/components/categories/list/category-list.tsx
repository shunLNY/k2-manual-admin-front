...
const CategoryList = (props: Props) => {
	...
	useEffect(() => {
		const selectedRoot = items.find(
			(c: any) => c.id === listCtx.selectedTabId,
		);
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		const findAndReorder = (currentItems: any[]) => {
			...
			for (const item of currentItems) {
				if (
					...
					findAndReorder(item.child_categories)
				) {
					return true;
				}
			}
		...
		if (foundList) {
			const oldIndex = foundList.findIndex(
	...
	return (
		<ListBody
			...
			extraHeaderContent={
				<button
					...
					onClick={() =>
						router.push(
							`/categories/new?parentId=${listCtx.selectedTabId}`,
						)
					}
				>
					+サブカテゴリ
		...
		>
				...
				{items.map((category: Category) => (
					<div
						...
						className={classNames(CategoryListStyles.tab_wrapper, {
							[CategoryListStyles.active]:
								listCtx.selectedTabId === category.id,
						})}
					>
						<button
							className={CategoryListStyles.tab_button}
							onClick={() =>
								listCtx.setSelectedTabId(category.id)
							}
						>
			...
			<div
				className={`${styles.table_wrapper} ${CategoryListStyles.drag_container}`}
			>
				<div
					className={`${styles.table_header} ${CategoryListStyles.list_header}`}
				>
            ...
            <Checkbox onChange={handleAllCheckboxChange} checked={listCtx.isAllChecked} />
					...
					<div role="button" className={styles.table_head}></div>
					<div className={styles.table_head}></div>
				</div>
				<div className={styles.list_wrapper}>
					...
					>
						<SortableContext
							items={list.map((i: { id: any }) => i.id)}
							strategy={verticalListSortingStrategy}
						>
							<ul
								className={`${styles.list_item} ${CategoryListStyles.sortable_list} noscroll`}
							>
						...
						<DragOverlay>
							{activeId ? (
								<div
									className={`${styles.item_wrapper} ${CategoryListStyles.item_wrapper} opacity-50`}
								>
									{/* Render a simplified version of the dragged item */}
									<div
										className={
											CategoryListStyles.drag_handle
										}
									>
										≡
									</div>
