...
// Form ထဲက data အမျိုးအစားကို သတ်မှတ်ရန် Interface
...
const EmailVerification = () => {
	...
	// React Hook Form ကို အသုံးပြုပြီး default value သတ်မှတ်ခြင်း
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	// Form Submit လုပ်သည့်အခါ အလုပ်လုပ်မည့် Function
	const onSubmitData = async (data: EmailFormInput) => {
		setIsLoading(true);
		setMessage("");

		try {
			// NestJS Backend Endpoint ဆီသို့ တိုက်ရိုက် လှမ်းခေါ်ခြင်း
			...
			// Security အာမခံချက်အရ အကောင့်ရှိမရှိကို အတိအကျမပြောဘဲ generic message သာ ပြသခြင်း
			setMessage(
		...
		<div className={styles.loginContainer}>
			<div className={styles.formContainer}>
				...
				<p className={styles.description}>
					パスワード再設定用リンクを送信するメールアドレスを
					<br />
					入力してください。
				</p>

				{/* React Hook Form ရဲ့ handleSubmit ကို ချိတ်ဆက်ခြင်း */}
				<form
				...
				>
					<div className={styles.inputContainer}>
						...
						{/* Custom TextField ထဲသို့ register ပေးပို့ပြီး Validation စစ်ဆေးခြင်း */}
						<TextField
							name="email"
							register={register}
							placeholder="メールアドレス"
					...
					{/* အီးမေးလ်ပုံစံ မှားယွင်းပါက Frontend ဘက်မှ ချက်ချင်း ပြသမည့် Error message စာသား */}
					{errors.email && (
						<p
							style={{
								color: "#e53e3e",
					...
					<div className={styles.btnContainer}>
						...
						<ButtonSave
							disabled={isLoading}
							className={`${isLoading ? `${styles.disable}` : ""}`}
							type="submit"
							text="送信する"
						/>
					</div>
