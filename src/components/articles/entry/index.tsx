...
const ArticleEntry = () => {
	...
	useEffect(() => {
		if (blogInfo?.id && _id && blogInfo.id === _id) {
			...
			const catName =
				info.category?.category_name || info.categoryName || "";
	...
	const handleConfirmSubmit = () => {
		...
		if (!hasStartDate && !hasEndDate) {
			...
			transformedData.published_start_at =
				transformedData.published_start_at;
			transformedData.published_end_at = null;
		} else if (!hasStartDate && hasEndDate) {
				...
				.then(({ image }: any) => {
					...
					fetcher("/api/proxy/files/_tmp/image/upload", {
						...
						.catch((err) => {
							toast.update(id, {
								render:
									err?.info?.message || "Image Upload Failed",
								type: "error",
								isLoading: false,
								autoClose: 1500,
							});
						...
						<FormControl label="サムネイル" any>
							<div className={styles.img_upload_container}>
								{watch("thumbnail_path") ? (
									...
									<>
										<div
											className={styles.thumbnail_upload}
										>
											<p className={styles.thumbnail_btn}>
												PHOTOS UPLOAD
											</p>
										</div>
										<input
